...
----------------------------------------------------------------------------------------------------
-- Set ArrangeDesktop
if spoon.ArrangeDesktop then
    spoon.ModalMgr:new("ArrangeDesktopM")
    local cmodal = spoon.ModalMgr.modal_list["ArrangeDesktopM"]
    cmodal:bind('', 'escape', 'Deactivate ArrangeDesktopM', function() 
        hs.notify.new({title="Tip", informativeText="退出ArrangeDesktop设置模式"}):send()
        spoon.ModalMgr:deactivate({"ArrangeDesktopM"}) 
    end)
    cmodal:bind('', 'Q', 'Deactivate ArrangeDesktopM', function() 
        hs.notify.new({title="Tip", informativeText="退出ArrangeDesktop设置模式"}):send()
        spoon.ModalMgr:deactivate({"ArrangeDesktopM"}) 
    end)
    cmodal:bind('', 'C', 'ArrangeDesktop createArrangement', function() 
        spoon.ArrangeDesktop:createArrangement()
        hs.notify.new({title="Tip", informativeText="退出ArrangeDesktop设置模式"}):send()
        spoon.ModalMgr:deactivate({"ArrangeDesktopM"}) 
    end)
    cmodal:bind('', 'A', 'ArrangeDesktop arrange', function() 
        spoon.ArrangeDesktop:addMenuItems(Office)
        spoon.ArrangeDesktop:arrange("Office")
        hs.notify.new({title="Tip", informativeText="退出ArrangeDesktop设置模式"}):send()
        spoon.ModalMgr:deactivate({"ArrangeDesktopM"}) 
    end)

    -- Register ArrangeDesktopM with modal supervisor
    hsarrangeDesktop_keys = hsarrangeDesktop_keys or {"alt", "B"}
    if string.len(hsarrangeDesktop_keys[2]) > 0 then
        spoon.ModalMgr.supervisor:bind(hsarrangeDesktop_keys[1], hsarrangeDesktop_keys[2], "Enter ArrangeDesktopM Environment", function()
            hs.notify.new({title="Tip", informativeText="进入ArrangeDesktop设置模式"}):send()
            spoon.ModalMgr:activate({"ArrangeDesktopM"})
        end)
    end
end
